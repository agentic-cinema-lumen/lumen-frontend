...
export default function AboutPage(){
  return <main className="about-page">
    ...
    <section className="architecture-section"><div className="architecture-heading"><div><p className="kicker">SYSTEM ARCHITECTURE</p><h2>One deterministic workflow.<br/>Many intelligent workers.</h2><p className="architecture-summary">Agentic model training on Google Cloud produces a versioned hit-or-miss model. At runtime, the site sends images, text and classifications to the prediction API; agents call the model and Parallel Search, then return the evidence behind the outcome.</p></div><div className="legend"><span><i className="g-blue"/> Google Cloud</span><span><i className="g-yellow"/> Parallel</span><span><i className="g-green"/> Model output</span></div></div>
      <div className="architecture-diagram">
        ...
        <div className="arch-column models"><p>04 · AGENTIC MODEL TRAINING</p><ArchCard icon={<BrainCircuit/>} title="Automated model trainer" copy="Google Cloud pipelines generate features, train models and emit versioned model artifacts" /><ArchCard icon={<Gauge/>} title="BigQuery ML" copy="Calibrates hit-or-miss likelihood against held-out outcomes" /><div className="loop-arrow">Evaluate → adjust → validate ↻</div></div>
        <FlowArrow label="serve" />
        <div className="arch-column serving"><p>05 · HIT OR MISS OUTPUT</p><ArchCard icon={<Cloud/>} title="Cloud Run prediction API" copy="Receives images, text and classifications for secure multimodal inference" /><div className="output-card"><b>82</b><div><span>Hit likelihood</span><small>87% confidence</small></div></div></div>
      </div>
    ...
    <section className="compliance-section"><div><p className="kicker">BUILT FOR AGENTIC CINEMA</p><h2>Each platform earns its place.</h2></div><div className="compliance-grid"><Rule title="Gemini multimodal intake" text="Gemini on Google Cloud reads scripts, images and classifications into structured creative signals."/><Rule title="Agentic model training" text="Vertex AI Pipelines and BigQuery ML continuously train and calibrate the hit-or-miss model."/><Rule title="Parallel live evidence" text="The Market Agent uses Parallel Search for comparable titles, demand, audience context and release signals."/><Rule title="Cloud-native serving" text="Cloud Run, Storage, IAM and Logging keep inference secure, observable and reproducible."/></div></section>
